...
const tableHeader =
document.querySelectorAll('#cashbackTable th');
...
'5px'; // Optional to keep rounded corners
...
const table =
document.getElementById("cashbackTable");
const rows =
...
rows.sort((a, b) => {
const aText =
a.cells[columnIndex].textContent.replace('₹', '').trim();
const bText =
b.cells[columnIndex].textContent.replace('₹', '').trim();
...
rows.forEach(row =>
table.tBodies[0].appendChild(row)); // Reattach sorted rows
}

// Ensure filters are added on page load
document.addEventListener('DOMContentLoaded', addSearchFilters);

// Function to toggle visibility of columns based on checkbox state
function toggleColumn(columnIndex) {
 const table =
     document.getElementById('cashbackTable');

 // Toggle visibility of header cell
 const headerCell =
     table.querySelector(`thead tr th:nth-child(${columnIndex + 1})`);
 const isVisible =
     headerCell.style.display !== 'none';

 // Set header cell display
 headerCell.style.display =
     isVisible ? 'none' : '';

 // Toggle visibility of all rows in that column
